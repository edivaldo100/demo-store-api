...
public class OrderedServiceImpl implements OrderedService {
	...
	public ResponseEntity<Response<OrderedDto>> register(OrderedDto orderedDto, BindingResult result,
		...
		try {
			...
			headers.setLocation(ucBuilder.path("/api/orders/{id}").buildAndExpand(newDto.getOrderNumber()).toUri());
			return new ResponseEntity<Response<OrderedDto>>(response, headers, HttpStatus.CREATED);
		} catch (Exception e) {
			log.info("Erro Default");
			response.getErrors().add("Erro interno, tente mais tarde.");
			return ResponseEntity.badRequest().body(response);
	...
	private void validaData(OrderedDto orderedDto, BindingResult result) {
		...
		if (findById != null)
			result.addError(new ObjectError("order", "Número de Pedido já cadastrado."));

		User findByIdUser = this.userRepository.findById(orderedDto.getUserId());
		if (findByIdUser == null)
			result.addError(new ObjectError("order", "Cliente com ID: " + orderedDto.getUserId() + " não cadastrado."));
			...
			productItemDToList.forEach(productItemDTo -> {
				...
				if (productDto != null) {
					...
					if (id != null) {
						Product product = productRepository.findById(id);
						if (product == null)
							result.addError(new ObjectError("product",
									"Produto com ID: " + productDto.getId() + " não cadastrado."));
					} else {
						result.addError(new ObjectError("product",
								"Produto com ID: " + productDto.getId() + " não cadastrado."));
					}
